...
(function () {
  ...
  function verifyOperatorCode(value) {
    ...
    return crypto.subtle.importKey("jwk", publicJwk, { name: "ECDSA", namedCurve: "P-256" }, false, ["verify"])
      .then(function (key) {
        ...
        if (!payload || payload.v !== 1 || !payload.exp || payload.exp <= now) throw new Error("That access code has expired.");
        if (payload.exp > now + 31 * 86400) throw new Error("That access code has an invalid expiry.");
  ...
  function createOperatorCode(privateJwk, scopes, seconds) {
    ...
    return crypto.subtle.importKey("jwk", privateJwk, { name: "ECDSA", namedCurve: "P-256" }, false, ["sign"])
      ...
      .then(function (sig) { return "v1." + encoded + "." + bytesToB64u(sig); });
  }
  ...
  function pickImage(cb) {
    ...
    inp.addEventListener("change", function () {
      ...
      reader.onload = function () {
        var img = new Image();
        img.onload = function () {
          /* small PNGs (logos with transparency) pass through untouched;
             everything else is downscaled so the export stays deployable */
          ...
          toast("Image embedded (" + Math.round(out.length / 1024) + " KB stored in content)");
        };
  ...
  function paneBuilder() {
    ...
    [["shipped", "Shipped"], ["motion", "In Motion"], ["next", "Up Next"]].forEach(function (pair) {
      ...
      p.appendChild(listEditor({
        path: "builder." + pair[0],
        itemLabel: function (x) { return x.name; },
        blank: function () { return { name: "New item", note: "" }; },
        fields: function (x, i, card) {
          return [
            objField(x, "name", "Name", { onInput: card.syncLabel }),
            objField(x, "note", "Note", { textarea: true })
          ];
        }
      }));
    });
    ...
    p.appendChild(listEditor({
      ...
      fields: function (x, i, card) {
        ...
        return [
          ...
          objField(x, "icon", "Icon (optional — emoji or short glyph shown on the card)", { onInput: card.syncLabel }),
          objField(x, "descMd", "Description", { md: true }),
          objField(x, "price", "Price"),
          objSelect(x, "frame", "Frame", ["square", "tall", "wide"]),
          objField(x, "paymentLink", "Payment link (optional — Stripe/PayPal URL)")
        ];
      }
    }));
    ...
    p.appendChild(listEditor({
      ...
      fields: function (x, i, card) {
        ...
        return [
          ...
          objField(x, "paymentLink", "Payment link (optional)")
        ];
      }
    }));
    ...
    p.appendChild(listEditor({
      ...
      fields: function (x, i, card) {
        ...
        return [
          ...
          objField(x, "icon", "Icon (optional — emoji or short glyph)", { onInput: card.syncLabel }),
  ...
  function genSitemap() {
    ...
    var urls = ["index.html", "sites.html", "apps.html", "photos.html", "blog.html", "builder.html", "estimate.html"].map(function (f) {
  ...
  function paneAnalytics() {
    ...
    steps.innerHTML =
      '<li><b>Enable two APIs</b> — sign into Google with the same account that owns your Analytics, then open each link and hit the blue <b>Enable</b> button: <a href="https://console.cloud.google.com/apis/library/analyticsdata.googleapis.com" target="_blank" rel="noopener">Analytics Data API ↗</a> · <a href="https://console.cloud.google.com/apis/library/analyticsadmin.googleapis.com" target="_blank" rel="noopener">Analytics Admin API ↗</a> (the second one lets this pane auto-find your property so you never touch an ID).</li>' +
  ...
  function paneSeo() {
    ...
    FACTS.registry.forEach(function (f) {
      p.appendChild(el("div", "fact-match",
        ...
        '<p><a href="' + MD.esc(f.url) + '" target="_blank" rel="noopener noreferrer">' + MD.esc(f.source) + " \u2197</a></p>"));
    });
  }
  ...
  function paneSettings() {
    ...
    p.appendChild(textField("Substack URL", "meta.substack"));
    p.appendChild(mediaField(bindPath("meta.logo"), "Logo", { fallbackNote: "NO LOGO SET" }));
    p.appendChild(mediaField(bindPath("meta.ogImage"), "OG / social share image", { fallbackNote: "NO OG IMAGE SET" }));
    ...
    passField.innerHTML = "<label>New passphrase (min 8 chars)</label>";
    ...
    passBtn.addEventListener("click", function () {
      ...
      if (v.length < 8) { toast("Too short — 8 characters minimum"); return; }
      sha256(v).then(function (h) {
        set("owner.passHash", h);
        saveDraft();
        passInput.value = "";
    ...
    if (publicKey && privateMatches && deployedReady) {
      ...
      [
        ...
        var op = el("option", null, x[0]); op.value = x[1]; if (x[1] === 86400) op.selected = true; expiry.appendChild(op);
      });
      ...
      makeLink.addEventListener("click", function () {
        ...
        createOperatorCode(privateKey, selected, Number(expiry.value)).then(function (token) {
          var base = location.href.split("#")[0];
          accessOut.value = base + "#access=" + token;
      ...
      p.appendChild(el("p", "dash-hint", "Scope limits the console interface for a trusted professional. Because this remains a static site, it is not a server-enforced authorization boundary. Regenerate and deploy the key to revoke every unexpired link immediately."));
    }
  ...
  // ── gate ──
  function openConsole(role, scopes, expiry) {
    ...
    if (CURRENT_ROLE === "operator") {
      var hours = Math.max(1, Math.ceil((expiry * 1000 - Date.now()) / 3600000));
      badge.textContent = "Temporary · " + hours + "h left";
    ...
    $("#btn-publish").addEventListener("click", function () {
      ...
      if (out.length > 1500000 && !confirm("Heads up: content.js is " + kb + " KB — lots of embedded images. It still works, but every page loads it, so pages get slower. Publish anyway?")) return;
  ...
  function boot() {
    ...
    function acceptOperator(value, remember) {
      return verifyOperatorCode(value).then(function (result) {
        ...
        openConsole("operator", result.payload.scopes, result.payload.exp);
        return true;
      });
    }
    ...
    form.addEventListener("submit", function (e) {
      ...
      sha256(pass).then(function (h) {
        var expected = (W.owner && W.owner.passHash) || (SHIP.owner && SHIP.owner.passHash);
        if (h === expected) {
